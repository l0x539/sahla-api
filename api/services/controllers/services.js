...
module.exports = {
  async find(ctx) {
    ...
    return entities.map(entity => sanitizeEntity(entity, { model: strapi.models.services }));
  },
  async create(ctx) {
    ...
    if (ctx.is('multipart')) {
      ...
      entity = await strapi.services.services.create(data, { files });
    } else {
      entity = await strapi.services.services.create(ctx.request.body);
    }
    return sanitizeEntity(entity, { model: strapi.models.services });
  },
  async delete(ctx) {

    let entity = await strapi.services.services.findOne({id: parseInt(ctx.params.id)});

    if (!ctx.state.user?.id || ctx.state.user?.id !== entity?.user_id) {
      return ctx.badRequest(`Not Allowed.`);
    }

    entity = await strapi.services.services.delete({id: parseInt(ctx.params.id)});
